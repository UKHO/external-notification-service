...
terraform plan -out "terraform.deployment.tfplan" -var elastic_apm_server_url=$elasticApmServerUrl -var elastic_apm_api_key=$elasticApmApiKey -var elastic_apm_environment=$elasticApmEnvironment -var elastic_apm_webjob_service_name=$elasticApmWebJobServiceName | tee terraform_output.txt
