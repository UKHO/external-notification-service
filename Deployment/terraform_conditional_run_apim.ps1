...
terraform init -backend-config="resource_group_name=$deploymentResourceGroupName" -backend-config="storage_account_name=$deploymentStorageAccountName" -backend-config="key=terraform.ens.apim.deployment.tfplan"
...
terraform plan -out "terraform.ens.apim.deployment.tfplan" | tee terraform_output.txt
...
terraform apply  "terraform.ens.apim.deployment.tfplan"
