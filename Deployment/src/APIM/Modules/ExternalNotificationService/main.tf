...
resource "azurerm_api_management_product_policy" "ees_product_policy" {
  ...
  xml_content = <<XML
	<policies>
	  <inbound>
      <base />
      <rate-limit calls="${var.ees_product_call_limit}" renewal-period="${var.ees_product_call_renewal_period}" retry-after-header-name="retry-after" remaining-calls-header-name="remaining-calls" />
